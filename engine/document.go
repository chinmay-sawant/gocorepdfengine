...
func GenerateDocument(cfg DocumentConfig) ([]byte, error) {
	...
	// === Content streams ===
	addPageContentStreams(isUA, d, cfg, contentIDs)

	// === Font ===
	addFontChain(isA4, d, &shared, cfg)
	...
	addStructureTree(isUA, d, nsRef, strRootRef, ptRef, elemDocID, elemPageIDs, pageIDs, lang, len(cfg.Pages))

	createCatalogDict(d, catalogID, pagesID, metaRef, strRootRef, oiRef, lang, isA4, isUA)

	return d.Build(), nil
}

func addPageContentStreams(isUA bool, d *doc.Document, cfg DocumentConfig, contentIDs []doc.ObjectID) {
...
func addFontChain(isA4 bool, d *doc.Document, shared *fontChain, cfg DocumentConfig) {
	if !isA4 {
	...
	if loadedFont != nil {
		addLoadedFontChain(d, loadedFont, cfg.UsedText, shared)
		return
	}

	addFallbackFontChain(d, shared)
}

func addStructureTree(isUA bool, d *doc.Document, nsRef, strRootRef, ptRef, elemDocID doc.ObjectID, elemPageIDs, pageIDs []doc.ObjectID, lang string, cfgPages int) {
	if isUA {
		d.AddObjectAt(nsRef, structure.Namespace())
		kids := make([]structure.StructElemKid, 0, cfgPages)
		ptMap := make(map[int][]doc.ObjectID, cfgPages)
...
func collectFontNames(pages []PageContent) map[string]bool {
...
func addLoadedFontChain(d *doc.Document, loadedFont *font.Font, usedText string, shared *fontChain) {
	...
	d.AddObjectAt(shared.descriptorID, font.DescriptorResourceDict(loadedFont, shared.fontFile2ID))
	d.AddObjectAt(shared.cidFontID, font.CIDFontDict(loadedFont, shared.descriptorID, shared.cidToGIDMapID))
	d.AddObjectAt(shared.fontRef, font.ResourceDict(libName, shared.cidFontID, shared.toUnicodeID))
}

func addFallbackFontChain(d *doc.Document, shared *fontChain) {
	fake := &font.Font{
	...
	d.AddObjectAt(shared.descriptorID, font.DescriptorResourceDict(fake, shared.fontFile2ID))
	d.AddObjectAt(shared.cidFontID, font.CIDFontDict(fake, shared.descriptorID, 0))
	d.AddObjectAt(shared.fontRef, font.ResourceDict(fake.Name, shared.cidFontID, shared.toUnicodeID))
}
